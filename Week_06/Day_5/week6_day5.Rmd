...
We would like to test whether the proportion of domesticated animal types in the population of animal types is greater than 5% (at a significance level α=0.05).

One-sample (indep) HT: proportions

Alpha = 0.05

H0: proportion of animals = 5
Ha: proportion of animals > 5
...
null_distribution2 %>% 
  ...
  shade_pvalue(obs_stat = observed_stat,
               direction = "both")
```
Conclusion - we can reject the null hypothesis and accept that it is significantly likely that the number of animals that are domesticated is likely to be greater than 5%.
...
#### Answer
The best test for this example would be the one-sample proportion using simulation to create a null distribution.
...
#### Answer
The best test for this example would be an independent two-sample test using proportions. Randomly permutatiing samples to create a null distribution for analysis.
...
#### Answer
This would be a one-sample mean where you will compare the sample to the known mean. Bootstrapping can be used here to repeat the process and determine if the size of the part has changed.
...
The p-value is greater than the selected significance level of 0.05 so we lack sufficient evidence to reject the null hypothesis. We do not have enough evidence that the proportion is statistically different from the null value.

### Question 2.

Website company problem. Significance level: 0.01, p-value: 0.006

The p-value is significantly smaller than the significance level - so we have found enough evidence in the sample to suggest that there is significant difference between the two website tests.

### Question 3.

Manufacturing company problem. Significance level: 0.05, p-value: 0.55

The p-value is higher than the significance level of 0.05 so we we lack sufficient evidence to reject the null hypothesis. Based on the sample, we do not have enough evidence that the average size of the car part is statistically significantly different from, less than or greater than the null value.
